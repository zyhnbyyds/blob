...
<template>
  <div class="flex justify-between text-14px">
    <NuxtLink
      :class="{ 'opacity-30': pre === null }"
      :to="pre ? pre._path : ''"
      hover="bg-my-20"
      class="w-40% border border-#e5e5e5 rounded-2 px-3 py-2 dark:border-#555"
      @click="handleChange(pre)"
    >
      <div class="flex">
    ...
    >
      ...
      <div
        style="text-overflow: ellipsis"
        class="overflow-hidden whitespace-nowrap text-right"
      >
        {{ next?.title || '无' }}
      </div>
    </NuxtLink>
