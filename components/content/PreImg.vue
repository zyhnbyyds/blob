...
<template>
  <div>
    <nuxt-img my-4 cursor-pointer :src="src" @click="modalVisible = true" />
    <Modal
      v-model:model-visible="modalVisible"
      top="20%"
      :bg-transparent="true"
    >
      <nuxt-img
      ...
      <template #footer>
        <div
          class="absolute-x-center bottom-10 flex gap-7 rounded-6 bg-#334 bg-op-20 bg-op-20 px-3 py-2 text-28px text-#fff"
        >
          <Icon
            class="cursor-pointer"
            name="solar:rounded-magnifer-zoom-in-outline"
            @click="handleScale('+')"
          />
          <Icon
            class="cursor-pointer"
          ...
          <Icon
            class="cursor-pointer"
            name="solar:rounded-magnifer-zoom-out-outline"
            @click="handleScale('-')"
          />
          <Icon
            class="cursor-pointer"
            name="iconoir:undo-circle"
            @click="rotateDeg -= 90"
          />
          <Icon
            class="cursor-pointer"
            name="iconoir:redo-circle"
            @click="rotateDeg += 90"
          />
        </div>
      </template>
