...
export function getStorages<R = object, E = object>(keys: ItemKeys, params?: ListOptions<E>): Promise<R> {
  ...
  const doGetFn = async (ids: ItemKeys) => {
    return new Promise((resolve, reject) => {
      Promise.all((ids ?? []).map(key => redis.getItem(key as string))).then((res) => {
        let list = res
